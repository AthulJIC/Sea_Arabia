        ...
        <View style={{marginTop: 15}}>
            <Pressable style={{width:'100%', height: selected ? 67 : 50, backgroundColor : 'rgba(247, 247, 249, 1)', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 2,elevation:2,borderTopLeftRadius:5,borderTopRightRadius:5, borderBottomLeftRadius: isOpen ? 0 : 5 , borderBottomRightRadius: isOpen ? 0 : 5 ,flexDirection:'row',alignItems:'center',paddingLeft:10}} onPress={destinationHandler}>
                ...
                <View style={{marginLeft:10,width:'75%'}}>
                    <Text style={{color:selected ? 'rgba(0, 0, 0, 0.8)' : 'rgba(0, 0, 0, 0.7)',fontSize:12,fontFamily:'Roboto-Regular',textAlign:'left',marginTop: selected ? 0 : 20}}>{selected ? selected?.place : 'Select Destination'}</Text>
                    <View style={{flexDirection:'row',marginTop:6}}>
                        <Text style={{color:'rgba(0, 0, 0, 0.8)',fontSize:12,fontFamily:'Roboto-Regular',textAlign:'left'}}>{selected?.time}</Text>
                        <Text style={{color:'rgba(0, 104, 117, 1)',fontSize:12,fontFamily:'Roboto-Regular',marginLeft:'auto',textAlign:'right'}}>{selected?.price}</Text>
                    </View>
                </View>
                ...
                <ScrollView scrollEnable={true} nestedScrollEnabled={true} keyboardShouldPersistTaps="always" style={{width:'auto', height:180,backgroundColor:'rgba(247, 247, 249, 1)', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 2,elevation:2,borderTopLeftRadius:isOpen ? 0 : 5,borderTopRightRadius:isOpen ? 0 : 5, borderBottomLeftRadius:5 , borderBottomRightRadius:5 ,paddingLeft:10}}>
                    {
                        ...
                        return (
                            <TouchableWithoutFeedback key={item.key} onPress={() => handleSelectSuggestion(item,index)} style={{
                                ...
                                <View>
                                    <Text style={{color:'rgba(0, 0, 0, 0.8)' ,fontSize:12,fontFamily:'Roboto-Regular',textAlign:'left',marginTop:10}}>{item.place}</Text>
                                    <View style={{flexDirection:'row',marginTop:6}}>
                                        <Text style={{color:'rgba(0, 0, 0, 0.8)',fontSize:12,fontFamily:'Roboto-Regular',textAlign:'left'}}>{item?.time}</Text>
                                        <Text style={{color:'rgba(0, 104, 117, 1)',fontSize:12,fontFamily:'Roboto-Regular',marginLeft:'auto',textAlign:'right',right:10}}>{item?.price}</Text>
                                    </View>
